...
const OM_NUMBER = "22606625715"; // Sans + pour WhatsApp
const ADMIN_WHATSAPP = "22606625715"; // Ton numéro pour recevoir notifs
...
async function renderDocs() {
  try {
    ...
    console.error(err);
    $('#loadingDocs').innerHTML = '<p class="empty">Erreur de chargement.</p>';
  }
}
...
function displayDocs(docs) {
  ...
  if (!grid) return;
  if (!docs.length) {
    grid.innerHTML = '<p class="empty" style="grid-column:1/-1">Aucun document trouvé</p>';
    return;
  }
...
// COMMANDE - VERSION AVEC VALIDATION
$('#checkoutForm')?.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    // 1. Créer la commande SANS envoyer email
    const orderRef = await db.collection('orders').add(orderData);
    
    // 2. Notifier l'admin sur WhatsApp
    const adminMsg = `🔔 NOUVELLE COMMANDE EDU DOCS\n\n` +
      `👤 Client: ${orderData.nom}\n` +
      `📧 Email: ${orderData.email}\n` +
      `📱 WhatsApp: ${orderData.telephone}\n` +
      `💳 ID Transaction: ${orderData.transactionId}\n` +
      `💰 Total: ${money(orderData.total)}\n\n` +
      `📚 Documents:\n${cart.map(i => `• ${i.title}`).join('\n')}\n\n` +
      `Valide sur: ${window.location.origin}/admin.html`;
...
// ADMIN - VALIDATION DES COMMANDES
        ...
        <div class="item-row" style="flex-direction:column;align-items:stretch;gap:12px">
          ...
          ${ord.status === 'pending' ? `
            <div style="display:flex;gap:8px">
              ...
              <button class="btn danger" onclick="rejectOrder('${o.id}')">❌ Refuser</button>
            </div>
          ` : ''}
        </div>
      `;
...
// VALIDER UNE COMMANDE = ENVOYER EMAIL + WHATSAPP
window.validateOrder = async (orderId) => {
  if (!confirm('Confirmer que le paiement Orange Money est reçu ?')) return;
  ...
  try {
    ...
    // 2. Préparer les liens de téléchargement
    ...
    const waMessage = `🎉 Bonjour ${order.nom} !\n\n` +
      ...
      `Voici tes documents Edu Docs :\n\n${waLinks}\n\n` +
      `Merci pour ta confiance ! 💪\n` +
      `Une question ? Réponds à ce message.`;
    
    // 5. Ouvrir WhatsApp pour envoyer au client
    const clientPhone = order.telephone.replace(/\D/g, ''); // Enlever espaces
    ...
    console.error(err);
    toast('Erreur lors de la validation : ' + err.message, 'error');
  }
};

window.rejectOrder = async (orderId) => {
  if (!confirm('Refuser cette commande ? Le client ne recevra rien.')) return;
  try {
