...
async function renderDocs() {
  try {
    ...
    const loading = $('#loadingDocs');
    if (loading) loading.innerHTML = '<p class="empty">Erreur de chargement. Vérifie les règles Firestore.</p>';
  }
}
...
function displayDocs(docs) {
  ...
  if (!grid) return;
  if (!docs.length) {
    grid.innerHTML = '<p class="empty" style="grid-column:1/-1">Aucun document disponible pour le moment</p>';
    return;
  }
