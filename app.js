...
// ATTEND QUE LE DOM SOIT PRÊT AVANT TOUT
document.addEventListener('DOMContentLoaded', () => {
  console.log('✅ DOM prêt - Initialisation...');
  
  // Init Firebase seulement après DOM ready
  ...
  async function renderDocs() {
    ...
    try {
      ...
      const loading = $('#loadingDocs');
      if (loading) loading.innerHTML = `<p class="empty">Erreur: ${err.message}</p>`;
    }
  }
  ...
  function displayDocs(docs) {
    const grid = $('#docsGrid');
    if (!grid) {
      console.log('⚠️ #docsGrid introuvable');
      return;
    }
    if (!docs.length) {
  ...
  async function loadAdminDocs() {
    try {
      ...
      if (docsCount) docsCount.textContent = docs.size;
